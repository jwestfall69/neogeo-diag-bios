...
Since ALL TESTS PASSED with diag bios, moved onto testing games with unibios.  Games in any/all slots just result in cross hatch.  This is an indication of a PROG rom address/data issue.

Go into unibios memory viewer, PROG rom data is 0x6100 for all addresses for all slots.
...
Tinned all exposed copper, repaired breaks, re-tested and still cross hatch in all slots.  However now the PROG rom data is all 0xFFFF.  Double checked my repairs, continuity was all good and no bridges.
...
74LS245's have the behavior if they are enabled but the input pins are dead they will output high on their output pins.  There should be 2 layers of 74LS245 chips between the PROG roms and the cpu, so maybe the ones nearest the PROG roms aren't getting enabled?

Based on that line of thinking use multi-meter to figure out where the PROG rom data lines go.  They go over to the [NEO-G0](https://wiki.neogeodev.org/index.php?title=NEO-G0)'s (which are basically 4x 74LS245s) on the left of the board.  Started logic probing them to check their enable pins, then I noticed this..
